...
public class LiquidacaoContratoService {
	...
	/**
	 * Regras de Negócios Regra Descrição da Regra Validação da Regra 1 -
	 * Portabilidade de Outra IF Caso o motivo da baixa for pelo motivo de
	 * Portabilidade de Outra IF, não devemos registrar na CIP Validar o não
	 * registro na CIP 2 - Refinanciamento de Carteira Caso o motivo da baixa for
	 * pelo motivo de Refinananciamento de Carteira, não devemos registrar na CIP
	 * Validar o não registro na CIP 3 - Liquidação no último mês do contrato Caso a
	 * baixa acontecer no último mês do contrato, não devemos registrar na CIP
	 * Validar o não registro na CIP 4 - Renegociação do Contrato Caso o motivo de
	 * baixa for pelo motivo de renegociação, devemos registrar na CIP Validar o
	 * registro na CIP 5 - Fraude Caso o motivo de baixa for pelo motivo de fraude,
	 * devemos registrar na CIP Validar o registro na CIP 6 - Inativação do Registro
	 * na CIP Caso o contrato seja cancelado por arrependimento do cliente devemos
	 * enviar um comando de exclusão do registro na CIP Validar a inativação do
	 * registro na CIP
	 * 
...
//		}
	...
	private void registrarNaCIP(ContratoPortabilidade contratoPortabilidade) {
		// 1.Montar Json com a respectiva operação (registro/inativação/alteração) pra
		// CIP
		// 2. Enviar Json
		...
		// 3. MS ao receber o retorno da CIP salva (criar uma coluna para cada item
		// abaixo):
		// a. Json de envio
